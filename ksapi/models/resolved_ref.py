...
class ResolvedRef(BaseModel):
    ...
    part_type: Optional[StrictStr] = None
    __properties: ClassVar[List[str]] = ["entity_type", "object_id", "display_name", "part_type"]

    @field_validator('entity_type')
    def entity_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['path_part', 'user']):
            raise ValueError("must be one of enum values ('path_part', 'user')")
        return value
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        # set to None if part_type (nullable) is None
        # and model_fields_set contains the field
        if self.part_type is None and "part_type" in self.model_fields_set:
            _dict['part_type'] = None

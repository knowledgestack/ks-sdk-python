...
class InputSnapshot(BaseModel):
    ...
    """ # noqa: E501
    path_part_id: UUID = Field(description="DOCUMENT_VERSION path_part of the pinned input version, or the FOLDER path_part of a live folder reference — see ``part_type``.")
    ...
    def part_type_validate_enum(cls, value):
        """Validates the enum"""
        if value not in set(['DOCUMENT_VERSION', 'FOLDER']):
            raise ValueError("must be one of enum values ('DOCUMENT_VERSION', 'FOLDER')")
        return value

...
class CreateSectionRequest(BaseModel):
    ...
    """ # noqa: E501
    name: Annotated[str, Field(min_length=1, strict=True, max_length=255)] = Field(description="Section name (can contain any characters)")
    parent_path_id: UUID = Field(description="Parent PathPart ID (must be DOCUMENT_VERSION or SECTION)")
    page_number: Optional[StrictInt] = Field(default=None, description="Page number in source document (must be > 0)")
    prev_sibling_path_id: Optional[UUID] = Field(default=None, description="PathPart ID to insert after (null = append to tail)")
    __properties: ClassVar[List[str]] = ["name", "parent_path_id", "page_number", "prev_sibling_path_id"]

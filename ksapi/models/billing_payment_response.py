...
class BillingPaymentResponse(BaseModel):
    ...
    plan_id: UUID = Field(description="Plan purchased.")
    interval: StrictStr = Field(description="Billing period purchased.")
    num_seats: StrictInt = Field(description="Seats purchased.")
    amount: StrictInt = Field(description="Total charged, in the currency's minor unit.")
    currency: StrictStr = Field(description="USD (cents) or CNY (fen).")
    invoice_url: Optional[StrictStr] = Field(default=None, description="Provider-hosted invoice/receipt, when available.")
    created_at: datetime = Field(description="When the payment confirmed.")
    __properties: ClassVar[List[str]] = ["id", "billing_system", "plan_id", "interval", "num_seats", "amount", "currency", "invoice_url", "created_at"]
